...
class LogitsProcessorOutput:
    ...
    topk_original_probs: Optional[torch.Tensor] = None
    topk_original_indices: Optional[torch.Tensor] = None
...
class LogitsProcessor(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            # assert last_index.min() >= 0, f"last_index contains negative: {last_index}"
            # assert last_index.max() < hidden_states.shape[0], f"last_index out of bounds: {last_index.max()} >= {hidden_states.shape[0]}"
