...
class VocabParallelEmbedding(torch.nn.Module):
    ...
    def weighted_forward(self, topk_probs: torch.Tensor, topk_indices: torch.Tensor) -> torch.Tensor:
        ...
        # Detect hard tokens: if subsequent values are -100, it's a standard token
        shard_mask = (topk_indices[:, 1:] == -100).all(dim=-1, keepdim=True)
        
        # Mask indices to handle out-of-bounds values (-100) before lookup
        # We reuse the first index for masked slots to keep it within range
        safe_indices = torch.where(topk_indices == -100, topk_indices[:, :1].expand_as(topk_indices), topk_indices)
        
        # Use quant_method.embedding for consistency
        topk_embeddings = self.quant_method.embedding(self, safe_indices.long())  # [B, K, D]
        
        # Optimization: for hard tokens, directly use the first embedding
        # For soft tokens, use the weighted sum
        # We perform weighted sum in float32 for precision
        soft_embs = torch.sum(topk_probs.unsqueeze(-1) * topk_embeddings, dim=1).to(topk_embeddings.dtype)
        hard_embs = topk_embeddings[:, 0, :]
        
        hidden_states = torch.where(shard_mask, hard_embs, soft_embs)
    ...
    def weighted_forward_tp(self, topk_probs: torch.Tensor, topk_indices: torch.Tensor) -> torch.Tensor:
        ...
        assert topk_probs.shape == topk_indices.shape, "topk_probs and topk_indices must have same shape."
        # Detect hard tokens: if subsequent values are -100, it's a standard token
        is_hard = (topk_indices[:, 1:] == -100).all(dim=-1, keepdim=True)
        ...
        # Calculate soft embeddings with FP32 precision
        soft_embs_parallel = torch.sum(
            topk_probs.unsqueeze(-1) * topk_embeddings, dim=1
        ).to(topk_embeddings.dtype)
        
        # Hard embedding shortcut (index 0)
        hard_embs_parallel = topk_embeddings[:, 0, :]
        
        # Select based on hard-token status
        hidden_states_parallel = torch.where(is_hard, hard_embs_parallel, soft_embs_parallel)
