...
global_server_args_dict = {
    ...
    "latent_end_token_id": ServerArgs.latent_end_token_id,
}
...
class Req:
    ...
    def __init__(
        ...
        # begin of latent reasoning
        # ==========
        self.sampling_params.latent_end_token_id = latent_end_token_id
        # ==========
        # end of latent reasoning
        # ==========
        ...
        if self.enable_latent:
            self.sampling_params.post_init_latent_mode()
            
            self.topk_prob = None
            self.topk_original_prob = None
            self.topk_gumbel = None
            self.topk_idx = None
            self.output_topk_prob_list = []
            self.output_topk_original_prob_list = []
            self.output_topk_original_indice_list = []
            self.output_topk_gumbel_list = []
            self.output_topk_idx_list = []
            self.output_topk_prob_list_tmp = []
            self.output_topk_original_prob_list_tmp = []
            self.output_topk_original_indice_list_tmp = []
    ...
    # begin of latent reasoning
    # ==========
    def update_latent_info(self, logits_output, index):
        

        if logits_output.topk_gumbels is not None:
            self.topk_gumbel = logits_output.topk_gumbels[index]
        else:
            self.topk_gumbel = None
        if logits_output.topk_original_probs is not None:
            self.topk_original_prob = logits_output.topk_original_probs[index]
        else:
            self.topk_original_prob = None
        self.topk_original_indice = logits_output.topk_original_indices[index]
        self.topk_prob = logits_output.topk_probs[index]
        self.topk_idx = logits_output.topk_indices[index]
        last_token_id = self.output_ids[-1] if self.output_ids else None

        if self.sampling_params.latent_mode:

            if self.sampling_params.latent_end_token_id == self.output_ids[-1]:

                self.sampling_params.latent_mode = False
                self.topk_prob[1:].fill_(0.0)
                self.topk_idx[1:].fill_(-100)
                self.topk_prob[0] = 1.0
                self.topk_idx[0] = self.sampling_params.latent_end_token_id
        ...
        if logits_output.topk_original_probs is not None:
            self.output_topk_original_prob_list_tmp.append(self.topk_original_prob)
        if logits_output.topk_original_indices is not None:
            self.output_topk_original_indice_list_tmp.append(self.topk_original_indice)
    ...
    def get_output_topk_original_prob_list(self):
        if self.output_topk_original_prob_list_tmp:
            self.output_topk_original_prob_list.extend(torch.stack(self.output_topk_original_prob_list_tmp, dim=0).cpu().tolist())
            self.output_topk_original_prob_list_tmp = []
        return self.output_topk_original_prob_list
    def get_output_topk_original_indice_list(self):
        if self.output_topk_original_indice_list_tmp:
            self.output_topk_original_indice_list.extend(torch.stack(self.output_topk_original_indice_list_tmp, dim=0).cpu().tolist())
            self.output_topk_original_indice_list_tmp = []
        return self.output_topk_original_indice_list
    # ==========
    # end of latent reasoning
    # ==========
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def init_new(
        ...
        return cls(
            ...
            latent_end_token_id=model_config.latent_end_token_id,
    ...
    def get_model_worker_batch(self) -> ModelWorkerBatch:
        ...
        topk_probs = None
        topk_indices = None
        if self.model_config.enable_latent:
