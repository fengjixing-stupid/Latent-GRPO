...
class SamplingBatchInfo:
    ...
    latent_modes: Optional[torch.Tensor] = None
    ...
    def from_schedule_batch(cls, batch: ScheduleBatch, vocab_size: int):
        ...
        noise_scales = torch.tensor(
            [r.sampling_params.noise_scale for r in reqs], dtype=torch.float
        ...
        add_noise_gumbel_softmax = torch.tensor(
            [r.sampling_params.add_noise_gumbel_softmax for r in reqs],
            dtype=torch.bool,
            device=device,
        )
        use_one_sided_gumbel_noise = torch.tensor(
            [r.sampling_params.use_one_sided_gumbel_noise for r in reqs],
            dtype=torch.bool,
            device=device,
        )
        # ==========
        ...
        # begin of latent reasoning
        # ==========
        enable_latent = batch.enable_latent
        if enable_latent:
            latent_modes = torch.tensor([req.sampling_params.latent_mode for req in reqs], dtype=torch.bool).to(device, non_blocking=True)
    ...
    def filter_batch(self, keep_indices: List[int], keep_indices_device: torch.Tensor):
        ...
        if self.enable_latent:
            filter_list.append("latent_modes")
            ...
            filter_list.append("noise_scales")
    ...
    def merge_batch(self, other: "SamplingBatchInfo"):
        ...
        # ==========
        if self.enable_latent:
            merge_list.append("latent_modes")
            ...
            merge_list.append("noise_scales")
