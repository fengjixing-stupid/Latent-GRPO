...
def collect_dp_compute_data_proto_with_observer(worker_group, output):
    """Concat the normal DataProto while preserving every rank observer packet."""
    rank_outputs = collect_dp_compute(worker_group, output)
    packets = []
    for worker_rank, rank_output in enumerate(rank_outputs):
        facts = rank_output.meta_info.get("latent_grpo_observer")
        if facts:
            packets.append({"worker_rank": worker_rank, **dict(facts)})
    combined = DataProto.concat(rank_outputs)
    if packets:
        combined.meta_info = dict(combined.meta_info)
        combined.meta_info["latent_grpo_worker_observers"] = packets
        combined.meta_info.pop("latent_grpo_observer", None)
    return combined


def _advance_actor_scheduler_after_update(scheduler, update_count):
    """Preserve one scheduler tick per outer update, except all-skipped updates."""
...
class ActorRolloutRefWorker(Worker):
    ...
    @register(
        dispatch_mode={
            ...
            "collect_fn": collect_dp_compute_data_proto_with_observer,
        }
    )
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            metrics["actor/lr"] = lr
            # A non-finite gradient skips optimizer.step(), so the scheduler
            # must not advance as if parameters had changed.
            update_count = self.actor.last_update_count
            _advance_actor_scheduler_after_update(self.actor_lr_scheduler, update_count)

            # TODO: here, we should return all metrics
            output = DataProto(meta_info={"metrics": metrics})
            observer_facts = self.actor.consume_latent_grpo_observer_facts()
            if observer_facts:
                output.meta_info["latent_grpo_observer"] = observer_facts
