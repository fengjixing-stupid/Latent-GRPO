...
def _validate_latent_runtime_config(config, model_hf_config):
    """Fail before SGLang startup when latent token metadata is unsafe."""
    enable_latent = config.get("enable_latent", False)
    latent_end_token_id = config.get("latent_end_token_id")
    vocab_size = getattr(model_hf_config, "vocab_size", None)
    ...
    if isinstance(latent_end_token_id, bool) or not isinstance(latent_end_token_id, int):
        if latent_end_token_id is None:
            raise ValueError("latent_end_token_id is required when latent mode is enabled")
        raise ValueError("latent_end_token_id must be an integer")
    ...
    if isinstance(vocab_size, bool) or not isinstance(vocab_size, int) or vocab_size < 1:
        raise ValueError("model vocab_size is required to validate latent_end_token_id")
    if latent_end_token_id >= vocab_size:
        raise ValueError("latent_end_token_id is outside model vocabulary")
    return {
        "enable_latent": True,
        "latent_end_token_id": latent_end_token_id,
        "vocab_size": vocab_size,
...
class SGLangRollout(BaseRollout):
    ...
    def _verify_config(self, model_hf_config):
        ...
        _validate_latent_runtime_config(self.config, model_hf_config)
