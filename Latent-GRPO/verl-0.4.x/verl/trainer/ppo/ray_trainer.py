...
def compute_advantage(
    ...
    **kwargs,
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        observer_enabled = observer is not None and observer.enabled
        if exclude_overlong_samples_from_advantage:
            advantage_fn = core_algos.compute_latent_grpo_outcome_advantage_firstmask_best_exclude_advantage
        else:
            advantage_fn = core_algos.compute_latent_grpo_outcome_advantage_firstmask_best_include_advantage
        advantage_kwargs = {
            "token_level_rewards": data.batch["token_level_rewards"],
        ...
        if exclude_overlong_samples_from_advantage:
            advantage_kwargs["max_response_length"] = grpo_calculation_mask.size(1)
        if observer_enabled:
            advantages, returns, id2stds, observer_data = advantage_fn(
                **advantage_kwargs,
                stable_group_ids=data.non_tensor_batch["group_id"],
                trajectory_ids=data.non_tensor_batch["trajectory_id"],
                return_observer_data=True,
            )
            for fact in observer_data:
                observer.emit("ocp_selection", fact)
        else:
            advantages, returns, id2stds = advantage_fn(**advantage_kwargs)
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
...
def _validate_latent_instrumentation_config(config, observer_enabled):
    """Reject paths whose latent/component alignment is not implemented."""
    actor = config.actor_rollout_ref.actor
    rollout = config.actor_rollout_ref.rollout
    instrumentation_enabled = bool(observer_enabled or rollout.get("enable_latent", False))
    if not instrumentation_enabled:
        return
    if actor.get("use_dynamic_bsz", False):
        raise ValueError("latent instrumentation requires actor.use_dynamic_bsz=false")
    if rollout.get("log_prob_use_dynamic_bsz", False):
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self._latent_grpo_observer = (
            load_observer_from_env(sink=observer_sink) if load_observer_from_env is not None else None
        )
    ...
    def _validate_config(self):
        ...
        _validate_latent_instrumentation_config(
            ...
            observer_enabled=bool(
                self._latent_grpo_observer is not None and self._latent_grpo_observer.enabled
            ),
        )
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if (
                emit_eval_question_facts is not None
                and self._latent_grpo_observer is not None
                and self._latent_grpo_observer.enabled
            ):
                reward_extra_info = result.get("reward_extra_info", {})
                correctness = reward_extra_info.get("acc")
                emit_eval_question_facts(
                    self._latent_grpo_observer,
                    ...
                    generation_ordinals=eval_generation_ordinals,
                )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    # Optional observer hook: stable IDs must be frozen before any
                    # union, balancing, filtering, accumulation, or reordering.
                    if attach_stable_ids_to_batch is not None and self._latent_grpo_observer is not None:
                        batch = attach_stable_ids_to_batch(
                            batch,
                            global_step=self.global_steps,
                            observer=self._latent_grpo_observer,
                        )
