...
class P1TorchCollectorTests(unittest.TestCase):
    ...
    def test_raw_generation_lengths_exclude_prompt_padding_and_do_not_count_topk_components(self):
        import torch
        from latent_grpo_runner.metrics.torch_collectors import collect_generated_trajectory_lengths
        ...
        class DataProtoLike:
            batch = {
                ...
                "rollout_topk_ids": torch.tensor(
                    [
                        [[10, 11, 12], [20, 21, 22], [30, -100, -100], [-100, -100, -100]],
                        [[40, 41, 42], [50, -100, -100], [-100, -100, -100], [-100, -100, -100]],
                    ]
                ),
            }

        self.assertEqual(collect_generated_trajectory_lengths(DataProtoLike()), [2, 2])
