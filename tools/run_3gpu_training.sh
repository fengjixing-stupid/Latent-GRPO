...
current_commit = __import__("subprocess").check_output(
...
accepted = (
    ...
    and payload.get("gpu_count") == 3
)
raise SystemExit(0 if accepted else 1)
PY

# Re-run the hardware/dependency/asset gate immediately before formal training;
# a previously accepted validation does not prove that the selected GPUs are
# still idle or that the environment has not drifted.
...
bash tools/run_3gpu_preflight.sh \
  ...
  --gpus "${GPUS}" \
  >"${OUTPUT_ROOT}.preflight.log" 2>&1 || blocked "formal_training_preflight_failed"

echo "3GPU_TRAINING_PREFLIGHT_GATE: PASS"
mkdir -p "${OUTPUT_ROOT}/logs"
mv "${OUTPUT_ROOT}.preflight.log" "${OUTPUT_ROOT}/logs/preflight.log"
...
manifest = {
    ...
    "wandb_mode": __import__("os").environ.get("WANDB_MODE", "offline"),
